            ...
            <div>
              ...
              {loadingMembers ? (
                ...
                <p className="text-xs text-gray-400 py-1">
                  No members with height data found. Add heights via member
                  profiles.
                </p>
              )}
            </div>
        ...
        {selected === "horizontal-jump" && (
          <div className="mb-4 bg-sky-50 border border-sky-100 rounded-xl px-3.5 py-3 flex items-start gap-2.5">
            ...
            <p className="text-xs text-sky-700 leading-relaxed">
              Camera must be set up perpendicular to the jump direction, framed
              so the full frame width equals exactly 10 metres.
            </p>
          </div>
        )}
        ...
        {selected === "step-length" && (
          <div className="mb-4 bg-teal-50 border border-teal-100 rounded-xl px-3.5 py-3 flex items-start gap-2.5">
            ...
            <p className="text-xs text-teal-700 leading-relaxed">
              Best results with a side-on camera angle at hip height. The
              subject should walk in a straight line across the frame.
            </p>
          </div>
        )}
        ...
        {selected === "lateral-shuffle" && (
          <div className="mb-4 bg-orange-50 border border-orange-100 rounded-xl px-3.5 py-3 flex items-start gap-2.5">
            ...
            <p className="text-xs text-orange-700 leading-relaxed">
              Best results with a front-facing camera at hip height capturing
              the full width of the shuffle. Subject should shuffle side-to-side
              across the frame.
            </p>
          </div>
        )}
...
function VideoDetailsContent() {
  ...
  const [latestFlyRun, setLatestFlyRun] =
    useState<AnalysisRunWithPoints | null>(null);
  const [latestLeapRun, setLatestLeapRun] = useState<VerticalLeapRun | null>(
    null,
  );
  const [latestHorizontalJumpRun, setLatestHorizontalJumpRun] =
    useState<HorizontalJumpRun | null>(null);
  const [latestStepLengthRun, setLatestStepLengthRun] =
    useState<StepLengthRun | null>(null);
  const [latestLateralShuffleRun, setLatestLateralShuffleRun] =
    useState<LateralShuffleRun | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      try {
        ...
        if (leapResult.status === "fulfilled")
          setLatestLeapRun(leapResult.value);
        if (horizontalJumpResult.status === "fulfilled")
          setLatestHorizontalJumpRun(horizontalJumpResult.value);
        if (stepLengthResult.status === "fulfilled")
          setLatestStepLengthRun(stepLengthResult.value);
        if (lateralShuffleResult.status === "fulfilled")
          setLatestLateralShuffleRun(lateralShuffleResult.value);
      } catch (err) {
        setError(
  ...
  const handleConfirmFlyRun = async () => {
    ...
    setProcessError("");
    try {
      const result = await api.processVideo(token, Number(videoId), {
        analysis_type: "fly-run",
      });
      setLatestFlyRun(result.run as AnalysisRunWithPoints);
  ...
  const handleConfirmVerticalLeap = async (heightCm: number) => {
    ...
    setProcessError("");
    try {
      const result = await api.processVerticalLeap(
        token,
        Number(videoId),
        heightCm,
      );
  ...
  const handleConfirmHorizontalJump = async () => {
    ...
    try {
      ...
      setShowProcessModal(false);
      router.push(
        `/videos/horizontal-jump?id=${videoId}&runId=${result.run.id}`,
      );
    } catch (err) {
      setProcessError(
        err instanceof ApiError ? err.message : "Analysis failed",
  ...
  const handleConfirmSingleLegHop = async () => {
    ...
    try {
      ...
      setShowProcessModal(false);
      router.push(
        `/videos/single-leg-hop?id=${videoId}&runId=${result.run.id}`,
      );
    } catch (err) {
      setProcessError(
        err instanceof ApiError ? err.message : "Analysis failed",
  ...
  const handleConfirmLateralShuffle = async () => {
    ...
    try {
      ...
      setShowProcessModal(false);
      router.push(
        `/videos/lateral-shuffle?id=${videoId}&runId=${result.run.id}`,
      );
    } catch (err) {
      setProcessError(
        err instanceof ApiError ? err.message : "Analysis failed",
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50 p-6">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          <div className="relative w-full aspect-video bg-gradient-to-br from-slate-700 to-slate-900 flex items-center justify-center">
            ...
            <span
              className={`absolute top-3 left-3.5 ${fmtColor} text-white text-xs font-bold rounded-md px-2.5 py-1 uppercase tracking-wide`}
            >
              {video.format || "—"}
            </span>
          </div>
              ...
              <div className="bg-blue-50 border border-blue-100 rounded-xl p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <p className="text-xs text-gray-400">
                    {formatDate(latestFlyRun.created_at)}
                  </p>
                </div>
                <div className="grid grid-cols-3 gap-2 mb-3">
                  {[
                    {
                      label: "Max",
                      value: `${latestFlyRun.max_speed_kmh} km/h`,
                    },
                    {
                      label: "Avg",
                      value: `${latestFlyRun.avg_speed_kmh} km/h`,
                    },
                    {
                      label: "Min",
                      value: `${latestFlyRun.min_speed_kmh} km/h`,
                    },
                  ].map(({ label, value }) => (
                    <div
                      key={label}
                      className="bg-white rounded-lg p-2.5 text-center border border-blue-100"
                    >
                ...
                <button
                  onClick={() =>
                    router.push(
                      `/videos/analysis?id=${videoId}&runId=${latestFlyRun.id}`,
                    )
                  }
            ...
            {latestLeapRun && (
              <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <p className="text-xs text-gray-400">
                    {formatDate(latestLeapRun.created_at)}
                  </p>
                </div>
                <div className="grid grid-cols-3 gap-2 mb-3">
                  {[
                    {
                      label: "Jump Height",
                      value: `${latestLeapRun.jump_height_cm} cm`,
                    },
                    {
                      label: "Flight Time",
                      value: `${latestLeapRun.flight_time_s} s`,
                    },
                    {
                      label: "Athlete Height",
                      value: `${latestLeapRun.height_cm} cm`,
                    },
                  ].map(({ label, value }) => (
                    <div
                      key={label}
                      className="bg-white rounded-lg p-2.5 text-center border border-emerald-100"
                    >
                      <p className="text-xs text-gray-400">{label}</p>
                      <p className="text-sm font-bold text-gray-800 mt-0.5">
                        {value}
                ...
                <button
                  onClick={() =>
                    router.push(
                      `/videos/vertical-leap?id=${videoId}&runId=${latestLeapRun.id}`,
                    )
                  }
              ...
              <div className="bg-sky-50 border border-sky-100 rounded-xl p-4">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-xs font-semibold text-sky-700 uppercase tracking-wide">
                      Latest Horizontal Jump
                    </p>
                  </div>
                  <p className="text-xs text-gray-400">
                    {formatDate(latestHorizontalJumpRun.created_at)}
                  </p>
                </div>
                <div className="grid grid-cols-2 gap-2 mb-3">
                  {[
                    {
                      label: "Jump Distance",
                      value: `${latestHorizontalJumpRun.jump_distance_cm} cm`,
                    },
                    {
                      label: "Flight Time",
                      value: `${latestHorizontalJumpRun.flight_time_s} s`,
                    },
                  ].map(({ label, value }) => (
                    <div
                      key={label}
                      className="bg-white rounded-lg p-2.5 text-center border border-sky-100"
                    >
                      <p className="text-xs text-gray-400">{label}</p>
                      <p className="text-sm font-bold text-gray-800 mt-0.5">
                        {value}
                ...
                <button
                  onClick={() =>
                    router.push(
                      `/videos/horizontal-jump?id=${videoId}&runId=${latestHorizontalJumpRun.id}`,
                    )
                  }
            ...
            {latestStepLengthRun && (
              <div className="bg-teal-50 border border-teal-100 rounded-xl p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <p className="text-xs text-gray-400">
                    {formatDate(latestStepLengthRun.created_at)}
                  </p>
                </div>
                <div className="grid grid-cols-2 gap-2 mb-3">
                  {[
                    {
                      label: "Step Count",
                      value: `${latestStepLengthRun.step_count}`,
                    },
                    {
                      label: "Avg Step Length",
                      value: `${round2(latestStepLengthRun.avg_step_length_cm)} cm`,
                    },
                    ...(latestStepLengthRun.avg_stride_length_cm > 0
                      ? [
                          {
                            label: "Avg Stride",
                            value: `${round2(latestStepLengthRun.avg_stride_length_cm)} cm`,
                          },
                        ]
                      : []),
                    ...(latestStepLengthRun.avg_cadence_steps_min > 0
                      ? [
                          {
                            label: "Cadence",
                            value: `${round2(latestStepLengthRun.avg_cadence_steps_min)} /min`,
                          },
                        ]
                      : []),
                  ].map(({ label, value }) => (
                    <div
                      key={label}
                      className="bg-white rounded-lg p-2.5 text-center border border-teal-100"
                    >
                      <p className="text-xs text-gray-400">{label}</p>
                      <p className="text-sm font-bold text-gray-800 mt-0.5">
                        {value}
                ...
                <button
                  onClick={() =>
                    router.push(
                      `/videos/step-length?id=${videoId}&runId=${latestStepLengthRun.id}`,
                    )
                  }
            ...
            {latestLateralShuffleRun && (
              <div className="bg-orange-50 border border-orange-100 rounded-xl p-4">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-xs font-semibold text-orange-700 uppercase tracking-wide">
                      Latest Lateral Shuffle
                    </p>
                  </div>
                  <p className="text-xs text-gray-400">
                    {formatDate(latestLateralShuffleRun.created_at)}
                  </p>
                </div>
                <div className="grid grid-cols-2 gap-2 mb-3">
                  {[
                    {
                      ...
                      value: `${latestLateralShuffleRun.shuffle_count}`,
                    },
                    {
                      label: "Symmetry",
                      value: `${round2(latestLateralShuffleRun.symmetry_pct)}%`,
                    },
                    {
                      label: "Avg Width",
                      value: `${round2(latestLateralShuffleRun.avg_shuffle_width_cm)} cm`,
                    },
                    {
                      label: "Avg Speed",
                      value: `${round2(latestLateralShuffleRun.avg_shuffle_speed_cm_s)} cm/s`,
                    },
                  ].map(({ label, value }) => (
                    <div
                      key={label}
                      className="bg-white rounded-lg p-2.5 text-center border border-orange-100"
                    >
                      <p className="text-xs text-gray-400">{label}</p>
                      <p className="text-sm font-bold text-gray-800 mt-0.5">
                        {value}
                ...
                <button
                  onClick={() =>
                    router.push(
                      `/videos/lateral-shuffle?id=${videoId}&runId=${latestLateralShuffleRun.id}`,
                    )
                  }
      ...
      {showProcessModal && (
        <ProcessModal
          onClose={() => {
            if (!processing) setShowProcessModal(false);
          }}
